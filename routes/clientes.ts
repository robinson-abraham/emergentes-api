...
router.post("/login", async (req, res) => {
    ...
    // Procura pelo cliente com o e-mail fornecido
    const cliente = await prisma.cliente.findUnique({
      where: { email }
    })
  
    // Se o cliente não for encontrado, retorna erro
    if (!cliente) {
      // Usamos 401 (Unauthorized) para não dar pistas se o email existe ou não
      return res.status(401).json({ erro: "Login ou senha incorretos" })
    }
  
    // Compara a senha enviada pelo formulário com a senha hash do banco
    const senhaCorreta = bcrypt.compareSync(senha, cliente.senha)
  
    // Se a senha não for a correta, retorna erro
    if (!senhaCorreta) {
      return res.status(401).json({ erro: "Login ou senha incorretos" })
    }
  
    // Se tudo estiver certo, retorna os dados do cliente (sem a senha)
    res.status(200).json({
      id: cliente.id,
      nome: cliente.nome,
      email: cliente.email,
